...
if '"net/http"' not in text:
    text = text.replace('\t"net/url"\n', '\t"net/http"\n\t"net/url"\n', 1)

text = text.replace(
    'ErrUpdateUnsupportedInContainer = infraerrors.BadRequest("UPDATE_UNSUPPORTED_IN_CONTAINER", "online update is disabled for Docker deployments; please replace the Docker image and restart the container")',
    'ErrUpdateUnsupportedInContainer = infraerrors.BadRequest("UPDATE_UNSUPPORTED_IN_CONTAINER", "Docker update helper is not configured; set DOCKER_UPDATE_URL and DOCKER_UPDATE_TOKEN")',
    1,
)
...
helper_anchor = """func isDockerDeployment() bool {
\tif _, err := os.Stat("/.dockerenv"); err == nil {
\t\treturn true
\t}
\tif strings.TrimSpace(os.Getenv("KUBERNETES_SERVICE_HOST")) != "" {
...
\tendpoint := strings.TrimSpace(os.Getenv("DOCKER_UPDATE_URL"))
\ttoken := strings.TrimSpace(os.Getenv("DOCKER_UPDATE_TOKEN"))
\tref := strings.TrimSpace(os.Getenv("DOCKER_UPDATE_REF"))
\tif endpoint == "" || token == "" {
\t\treturn ErrUpdateUnsupportedInContainer
\t}
\tif ref == "" {
...
\t\treturn infraerrors.BadRequest("DOCKER_UPDATE_FAILED", "Docker update workflow rejected the update request")
\t}
\treturn nil
}
"""
if "func (s *UpdateService) performDockerUpdate" not in text and helper_anchor in text:
    text = text.replace(helper_anchor, helper_anchor + docker_helper, 1)
...
text = text.replace(
    'v-else-if="hasUpdate && isDockerDeployment" class="space-y-2"',
    'v-else-if="false && hasUpdate && isDockerDeployment" class="space-y-2"',
    1,
)
text = text.replace(
    ':href="releaseInfo.html_url"',
