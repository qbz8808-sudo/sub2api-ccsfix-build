<template>
  <AppLayout>
    ...
    />
    <!-- CC Switch fallback dialog: protocol import failed or is unavailable -->
    <BaseDialog
      :show="showCcsFallback"
      title="CC Switch 备用导入"
      width="wide"
      @close="closeCcsFallback"
    >
      <div v-if="ccsFallback" class="space-y-4">
        <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800 dark:border-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-200">
          没有检测到系统协议处理程序，已自动把 API Key 复制到剪贴板。你可以继续用下面任意一种方式完成导入。
        </div>

        <div class="grid gap-3 sm:grid-cols-2">
          <button class="btn btn-primary" @click="copyRawText(ccsFallback.apiKey, '已复制 API Key')">
            复制 API Key
          </button>
          <button class="btn btn-secondary" @click="copyRawText(ccsFallback.deeplink, '已复制一键导入链接')">
            复制一键导入链接
          </button>
          <button class="btn btn-secondary" @click="copyRawText(ccsFallback.manualConfig, '已复制手动配置')">
            复制手动配置
          </button>
          <button class="btn btn-secondary" @click="downloadCcsManualConfig">
            下载配置文本
          </button>
        </div>

        <div class="space-y-2">
          <p class="text-sm font-medium text-gray-900 dark:text-white">手动导入步骤</p>
          <ol class="list-decimal space-y-1 pl-5 text-sm text-gray-600 dark:text-gray-400">
            <li>打开 CC Switch。</li>
            <li>新建供应商，应用类型选择：{{ ccsFallback.appLabel }}。</li>
            <li>把下面的请求地址和 API Key 填进去。</li>
            <li>用量查询选择自定义，粘贴下面配置里的脚本。</li>
          </ol>
        </div>

        <div class="rounded-xl bg-gray-900 p-4 text-sm text-gray-100">
          <pre class="overflow-x-auto whitespace-pre-wrap break-all"><code>{{ ccsFallback.manualConfig }}</code></pre>
        </div>
      </div>

      <template #footer>
        <div class="flex justify-end">
          <button class="btn btn-secondary" @click="closeCcsFallback">关闭</button>
        </div>
      </template>
    </BaseDialog>
...
const ccsFallback = ref<CcsFallbackData | null>(null)
...
const copyRawText = async (text: string, message = '已复制') => {
  const success = await clipboardCopy(text, message)
  if (!success) {
    appStore.showError('复制失败，请手动选中内容复制')
  }
}
...
const executeCcsImport = (row: ApiKey, clientType: CcSwitchClientType) => {
  ...
  const usageAuthHeader = `Bearer ${row.key}`
  const platform = row.group?.platform || 'anthropic'
  ...
  try {
    ...
    setTimeout(() => {
      if (document.hasFocus()) {
        openCcsFallback(row, { baseUrl, platform, clientType, providerName, deeplink, usageScript })
      }
    }, 100)
  } catch (error) {
    openCcsFallback(row, { baseUrl, platform, clientType, providerName, deeplink, usageScript })
...
const getCcsAppLabel = (platform: GroupPlatform | string, clientType: CcSwitchClientType) => {
  if (platform === 'openai') return 'Codex'
  if (platform === 'gemini') return 'Gemini'
  if (platform === 'antigravity') return clientType === 'gemini' ? 'Gemini' : 'Claude'
  return 'Claude'
}

const getCcsEndpoint = (baseUrl: string, platform: GroupPlatform | string) => {
  const trimmed = baseUrl.replace(/\/+$/, '')
  return platform === 'antigravity' ? `${trimmed}/antigravity` : trimmed
}

const openCcsFallback = async (
  row: ApiKey,
  input: {
    baseUrl: string
  ...
  const endpoint = getCcsEndpoint(input.baseUrl, input.platform)
  const appLabel = getCcsAppLabel(input.platform, input.clientType)
  const manualConfig = [
    `名称：${input.providerName}`,
    `应用类型：${appLabel}`,
    `请求地址：${endpoint}`,
    `API Key：${row.key}`,
    '',
  ...
  ccsFallback.value = {
    ...
    usageScript: input.usageScript,
    manualConfig
  }
  showCcsFallback.value = true
  await copyRawText(row.key, '已复制 API Key，可手动粘贴到 CC Switch')
}

const closeCcsFallback = () => {
  showCcsFallback.value = false
  ccsFallback.value = null
}

const downloadCcsManualConfig = () => {
  if (!ccsFallback.value) return
  const blob = new Blob([ccsFallback.value.manualConfig], { type: 'text/plain;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = `${ccsFallback.value.providerName || 'sub2api'}-ccswitch-import.txt`
